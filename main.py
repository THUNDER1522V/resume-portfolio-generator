...
GEMINI_MODEL = "gemini-2.5-flash"  # swap for whatever model your instructor approves
...
def render_skills(skills: list) -> str:
    if not skills:
        return ""
    items = "".join(f"<li>{escape(s)}</li>" for s in skills)
    return f'<section class="section" id="skills"><h2>Skills</h2><ul class="pill-list">{items}</ul></section>'
...
def render_education(education: list) -> str:
    ...
    for edu in education:
        items += f"""
        <div class="entry">
          <h3>{escape(edu.get("degree", ""))}</h3>
          <p class="entry-meta">{escape(edu.get("institution", ""))} &middot; {escape(edu.get("years", ""))}</p>
        </div>"""
    return f'<section class="section" id="education"><h2>Education</h2>{items}</section>'
...
def render_experience(experience: list) -> str:
    ...
    for exp in experience:
        ...
        items += f"""
        <div class="entry">
          <h3>{escape(exp.get("role", ""))}</h3>
          <p class="entry-meta">{escape(exp.get("company", ""))} &middot; {escape(exp.get("dates", ""))}</p>
          {details_html}
        </div>"""
    return f'<section class="section" id="experience"><h2>Experience</h2>{items}</section>'
...
def render_projects(projects: list) -> str:
    ...
    for proj in projects:
        ...
        tech_html = f'<p class="entry-meta">{escape(tech)}</p>' if tech else ""
        items += f"""
        <div class="entry">
          <h3>{escape(proj.get("title", ""))}</h3>
          <p>{escape(proj.get("description", ""))}</p>
          {tech_html}
        </div>"""
    return f'<section class="section" id="projects"><h2>Projects</h2>{items}</section>'
...
def render_achievements(achievements: list) -> str:
    ...
    return f'<section class="section" id="achievements"><h2>Achievements</h2><ul>{items}</ul></section>'


def render_contact(contact: dict) -> str:
    links = []
    if contact.get("email"):
        links.append(f'<a href="mailto:{escape(contact["email"])}">Email</a>')
    if contact.get("phone"):
        links.append(f'<span>{escape(contact["phone"])}</span>')
    if contact.get("linkedin"):
        links.append(f'<a href="{escape(contact["linkedin"])}" target="_blank" rel="noopener">LinkedIn</a>')
    if contact.get("github"):
        links.append(f'<a href="{escape(contact["github"])}" target="_blank" rel="noopener">GitHub</a>')
    for link in contact.get("links", []):
        links.append(f'<a href="{escape(link)}" target="_blank" rel="noopener">{escape(link)}</a>')

    if not links:
        return ""
    return f'<section class="section" id="contact"><h2>Contact</h2><div class="contact-links">{" &middot; ".join(links)}</div></section>'
